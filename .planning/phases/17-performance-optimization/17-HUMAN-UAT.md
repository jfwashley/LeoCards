...
### 1. D-05 — four Motion→CSS keyframe swaps, plus the end-screen exit Link swap, are visually indistinguishable from their prior implementations
expected: Visit each of the four swapped components (study-session mount fade on entering /study, card-list accordion open/close on /deck/browse, ac-progress indeterminate bar wherever it renders, habitat-teaser glow pulse on the marketing/welcome surface) and confirm each CSS keyframe animation reads as pixel/motion-identical to its prior Motion (framer-motion/motion-react) implementation — same easing feel, same duration, same visual character. Repeat with `prefers-reduced-motion` enabled (OS or DevTools emulation) and confirm each component's reduced-motion branch still behaves correctly (no animation, or the documented reduced fallback), matching the prior Motion-based reduced-motion behavior. Additionally (added post-review, 2026-07-20): the study end-screen's "Back to deck" control was swapped from a button+`router.push()` to a `<Link>` as part of the PERF-04 nav-outlier fix (commit `e3b161e`) — confirm this control is pixel-identical to the prior button (same visual frame, spacing, hover/focus/active states, touch target) and that it still exits the study session correctly.
result: [pending]
note: Carried forward from 17-04 (deferred there); the CSS swaps were implemented in 17-04 Task 2 and are code-complete. The "Back to deck" Link swap was added in the 2026-07-20 post-review PERF-04 investigation (see 17-05-SUMMARY.md addendum). Josh has not yet done the side-by-side visual pass covering either.
...
result: [pending]
note: UPDATED post-review (2026-07-20, commit `0b5dc8d`): the `router.refresh()` originally wired in study-session.tsx (17-05 Task 2) was REMOVED as redundant — it was serializing a second server round trip after `router.push()` and doubling the study→dashboard nav latency (part of the PERF-04 outlier fix; see 17-05-SUMMARY.md addendum). Freshness is now guaranteed without it: `/dashboard` and `/habitat` are genuinely dynamic Server Component renders on an un-prefetched push, so the destination re-fetches due-count/habitat state fresh from the DB on every landing regardless of a client-side refresh call. This check should confirm the due-count/habitat state is STILL immediately correct on arrival with `refresh()` gone — i.e. that the dynamic render alone is sufficient freshness mechanism, not that a refresh call is doing the work. Card-add freshness (b) is unchanged, still via existing `revalidatePath()` Server Actions. Needs a manual live-browser confirmation since this is a landing-render-timing behavior not fully capturable by the automated e2e content-visible probe.
