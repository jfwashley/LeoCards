...
## Addendum — post-review PERF-04 investigation (2026-07-20)

**This addendum SUPERSEDES the "PERF-04 accepted-miss" verdict recorded above. PERF-04 is now MET at a re-baselined 850ms gate.**

1. **Code review fallout.** `17-REVIEW.md` (commit `cbc57ed`) found 3 Critical + 4 Warning issues, all fixed across commits `de1a3db`, `44ad5f4`, `2a57ebd`, `e0828c1`, `f38b1f0`, `bf8de40`, `20ab3c7`, `65cd8b1` (docs recorded in `1b9087a`). Two of the Critical findings (CR-02/CR-03) invalidated the original PERF-04 measurements above — an ~8s timeout artifact was baked into most of the original samples, so the 1.3-8s figures in this summary's "PERF-04 Measured Result" table are not trustworthy and should be read as superseded.
2. **Corrected re-measurement.** A full 6-pair re-measurement against a local prod build, taken after the review fixes, showed five pairs at ~480-650ms and one outlier: study→dashboard at 1,545ms (web) / 1,949ms (mobile). Josh directed further investigation until study→dashboard was under 850ms.
3. **Root causes found (nav-profile instrumentation, since reverted) and fixed in commits `0b5dc8d` + `e3b161e`:**
   - A redundant `router.refresh()` immediately after `router.push()` at both study-exit callsites serialized a second server round trip. Removed (`0b5dc8d`) — D-17 landing-freshness is still guaranteed because `/dashboard` and `/habitat` are dynamic renders on un-prefetched navs, so the destination re-fetches fresh data regardless.
   - The Neon HTTP driver rides Node's `fetch` (undici), whose default agent drops idle sockets after ~4s; any pause longer than that (e.g. a study session) forced the next dashboard render to pay fresh TCP+TLS handshakes inside its `decks`/`lang`/`habitatFacts` `Promise.all` (+~400ms, measured 540ms warm → 150ms cold-vs-warm delta). Fixed with a dedicated `undici` `Agent` dispatcher with a 60s keep-alive in `src/db/index.ts` (transitive dependency already present, no new package added; Node-runtime-only, guarded by try/catch).
   - Measurement skew: the study end screen mounts inside the `ss-fade-up` 0.4s CSS animation, and Playwright's click was waiting out the animation inside the timed window (~400ms per sample). Fixed with a trial-click before starting the timer. The end-screen "Back to deck" control was also swapped from a button+`router.push()` to a pixel-identical `<Link>` (correct navigation semantics, immediate dispatch, no behavioral or visual change).
   - Both fixes landed in commits `0b5dc8d` and `e3b161e`.
4. **D-15 gate re-baselined.** Per Josh's directive (2026-07-20), `e2e/13-perf.spec.ts`'s `NAV_GATE_MS` moved from 100 to **850**. 100ms is architecturally unreachable for these routes (genuinely dynamic RSC, real server render per navigation, ~600ms warm floor including Neon round-trip time).
5. **Official gated result (2026-07-20): PERF-04 nav gate GREEN.** Web 2/2 passed, mobile 2/2 passed against a local prod build; all six pairs measured ~470-690ms (study→dashboard median ~670ms). **PERF-04 is now MET at the re-baselined 850ms gate** — this supersedes the "accepted miss" verdict recorded earlier in this summary. The original <100ms instant-nav aspiration moves to backlog, gated behind a future PPR/Cache Components adoption and its own D-07 checkpoint.
6. **Regression evidence.** Full `vitest` run: 2174 passed after all changes. Full e2e suite re-ran clean post-review-fixes: web 71/71 passed; mobile green (2 dev-server-only compile-flake failures seen in `10-mobile-responsive` reproduced 6/6 PASS against the prod build, confirmed environmental noise, not app regressions).
7. **Guards held throughout.** Regression and schema-drift gates stayed clean; the immutable Phase 16 baseline (`.planning/phases/16-performance-baseline-measure/baseline/`) was untouched across the entire investigation.
