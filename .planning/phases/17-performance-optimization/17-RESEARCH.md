...
1. **Does D-02's poster-first requirement apply to a dashboard hero that doesn't currently show ambient video at all?**
   - **RESOLVED:** 17-03-PLAN.md Task 2 encodes the resolution procedure — the executor reads the `design/` dashboard handoff before touching media; default per Assumption A1 is that the existing static medallion already satisfies D-02 (dashboard LCP 1816 ms passes) and NO new media is added (D-12 no gold-plating). Only an explicit design call for an ambient dashboard clip triggers mirroring `habitat-video.tsx`'s poster-first pattern.
...
2. **Where exactly does the D-15 "content visibly rendered" signal get instrumented for PERF-04?**
   - **RESOLVED:** 17-01-PLAN.md Task 3 establishes the `data-perf-ready="true"` per-route real-content marker convention plus the `e2e/perf-markers.ts` `waitForPerfReady` helper; 17-03/17-04 apply the marker to each route's real-content root, and the Wave-5 nav gate (17-05) polls it as the D-15 passing signal (skeletons never carry the attribute).
...
3. **Should the route-filter flag for `measure-cwv.mjs` accept multiple routes at once, or exactly one?**
   - **RESOLVED:** 17-01-PLAN.md Task 1 implements comma-separated multi-route filtering (per the recommendation below), including the `/habitat` union needed for the D-11 spot-check — so D-08 shared-infrastructure batches can re-measure all affected routes in one run.
