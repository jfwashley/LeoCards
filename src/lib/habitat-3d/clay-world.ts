...
export type ClayMatFactory = (color: string) => THREE.MeshToonMaterial;
...
export interface ClayWorld {
  ...
  featureGroups: Record<string, THREE.Group>;
...
export function buildClayWorld(
  ...
  ctx.scene.background = new THREE.Color(sky.bot);
  if (!ctx.scene.fog) {
    ctx.scene.fog = new THREE.Fog(sky.fog, 36, 80);
  } else {
    (ctx.scene.fog as THREE.Fog).color.set(sky.fog);
  }
  ...
  const mat: ClayMatFactory = (c) => {
    const m = new THREE.MeshToonMaterial({ color: c, gradientMap: grad });
    materials.push(m);
    return m;
  };
  ...
  const skyMat = new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      bot: { value: new THREE.Color(sky.bot) },
      // Test 3 hook: alias `color1` to `top` for assertions.
      color1: { value: new THREE.Color(sky.top) },
    },
    vertexShader:
  ...
  fill.translateOnAxis(new THREE.Vector3(-5, 6, -3).normalize(), 8);
  root.add(fill);
  root.add(new THREE.HemisphereLight("#cfe6f0", "#88a5b0", 0.65));
  root.add(new THREE.AmbientLight("#ffffff", 0.22));
  ...
  const ring = new THREE.Mesh(
    new THREE.TorusGeometry(R * ISLAND_XZ - 0.6, 0.4, 14, 96),
    mat(CLAY.groundDeep),
  );
  ...
  // -------- feature groups --------
  const featureGroups: Record<string, THREE.Group> = {};
  function fg(name: string): THREE.Group {
  ...
  // -------- lake --------
  let lakeMat: THREE.MeshToonMaterial | null = null;
  const lilies: Array<{ obj: THREE.Group; phase: number; baseY: number }> = [];
  if (features.lake) {
    const lakeG = fg("lake");
    function curvedDisk(
      rx: number,
    ...
    const lake = new THREE.Mesh(curvedDisk(lakeRX, lakeRZ, 0.04), lakeMat);
    lake.receiveShadow = true;
    lakeG.add(lake);
    const lakeIn = new THREE.Mesh(
  ...
  // -------- trees --------
  const trees: Array<{ obj: THREE.Group; phase: number; freq: number }> = [];
  if (features.trees) {
    const treesG = fg("trees");
    function tree(x: number, z: number, scale: number): THREE.Group {
      const g = new THREE.Group();
      const trunk = new THREE.Mesh(
        new THREE.CylinderGeometry(0.2 * scale, 0.3 * scale, 1.2 * scale, 14),
        mat("#7a4f30"),
      );
      ...
      for (const p of puffs) {
        const m = new THREE.Mesh(
          new THREE.SphereGeometry(p.r * scale, 18, 14),
          mat(p.c),
        );
  ...
  if (features.flowers) {
    ...
    function flower(x: number, z: number, c: string): THREE.Group {
      const g = new THREE.Group();
  ...
  if (features.mushrooms) {
    ...
    function mushroom(x: number, z: number, s: number): THREE.Group {
      const g = new THREE.Group();
      const stem = new THREE.Mesh(
        new THREE.CylinderGeometry(0.1 * s, 0.13 * s, 0.35 * s, 10),
  ...
  if (features.rocks) {
    ...
    function rock(x: number, z: number, s: number): THREE.Mesh {
      const r = new THREE.Mesh(
        new THREE.SphereGeometry(0.4 * s, 12, 10),
        mat("#a8a098"),
      );
  ...
  if (features.cave) {
    const caveG = fg("cave");
    function buildCave(): THREE.Group {
      const g = new THREE.Group();
      const S = 1.75;
      const E = 2.0;
      ...
      const mouthMat = new THREE.MeshBasicMaterial({ color: "#0a0608" });
      ...
      for (const [x, z] of pebPos) {
        const peb = new THREE.Mesh(
          new THREE.SphereGeometry(0.13 * S, 12, 10),
          mat("#a8a098"),
        );
  ...
  if (features.clouds) {
    function cloud(x: number, y: number, z: number, s: number): THREE.Group {
      const g = new THREE.Group();
      ...
      for (const [dx, dy, dz] of offsets) {
        const m2 = new THREE.Mesh(
          new THREE.SphereGeometry(0.7 * s, 14, 10),
          mat(CLAY.cloud),
        );
  ...
  // -------- toys --------
  let toyG: THREE.Group | null = null;
  const toyX = 3.6;
  const toyZ = 2.4;
  ...
  if (features.toy) {
    ...
    for (let i = 0; i < 4; i++) {
      const stripe = new THREE.Mesh(
        new THREE.TorusGeometry(0.41, 0.045, 8, 28),
        mat(CLAY.toyStripe),
      );
    ...
    const soccerBody = new THREE.Mesh(
