 ...
 */
function imageToDataUrl(filePath: string): { image: string; mimeType: string } {
...
describe.skipIf(!RUN_EVALS)("Extraction eval — reference dataset", () => {
  ...
  it("fixtures directory and reference-labels.json exist when evals are run", () => {
    ...
    const fixtureKeys = Object.keys(raw).filter((k) => !k.startsWith("_"));
  ...
  it("D5a — no-words correctness: blank/no-text images return empty word array", {
    timeout: 60_000,
  }, async () => {
    ...
    for (const [filename, entry] of noTextEntries) {
      ...
      expect(
        result.words,
        `D5a FAIL for ${filename}: expected [] but got ${JSON.stringify(result.words)}`,
      ).toEqual([]);
    }
  });

  it("D3 + D4 — orthographic faithfulness + verbatim surface form: expected words present in output", {
    timeout: 300_000,
  }, async () => {
    expect(existsSync(fixturesDir)).toBe(true);
    expect(existsSync(labelsPath)).toBe(true);
    ...
    const textEntries = Object.entries(labels).filter(
      ([key, entry]) => !key.startsWith("_") && entry.expectedWords.length > 0,
    );
  ...
  it("D1 + D2 + D5b — manual rubric: log extraction outputs for tutor review", {
    timeout: 600_000,
  }, async () => {
    expect(existsSync(fixturesDir)).toBe(true);
    expect(existsSync(labelsPath)).toBe(true);
    ...
    for (const [filename, entry] of allEntries) {
      ...
      console.log(
        `    detectedLanguage: ${result.detectedLanguage ?? "(not returned)"}`,
      );
